...
class RentPayment(Document):
	...
	def validate(self):
		...
		if month.month == 12:
			next_month = month_start.replace(year=month.year + 1, month=1)
		else:
			next_month = month_start.replace(month=month.month + 1)
	...
	def create_sales_invoice(self):
		...
		invoice.posting_date = self.payment_date or frappe.utils.today()

		invoice.append("items", {"item_code": "Rent", "qty": 1, "rate": self.amount})
	...
	def create_payment_entry(self, invoice):
		...
		receivable_account = frappe.db.get_value(
			"Account", {"company": invoice.company, "account_type": "Receivable", "is_group": 0}, "name"
		)
		...
		paid_from = frappe.db.get_value(
			"Account",
			{"company": invoice.company, "account_type": ["in", ["Bank", "Cash"]], "is_group": 0},
			"name",
		)
		...
		frappe.msgprint(f"Payment Entry {payment.name} created successfully.")
