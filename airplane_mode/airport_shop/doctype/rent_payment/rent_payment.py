...
class RentPayment(Document):
    ...
    def create_sales_invoice(self):
        ...
        if not customer:
            frappe.throw(
                _("Please select a Customer in the Tenant's Company field.")
            )
        ...
        invoice.append(
            "items",
            {"item_code": "Rent", "qty": 1, "rate": self.amount},
        )
        ...
        frappe.msgprint(
            _("Sales Invoice {0} created successfully.").format(invoice.name)
        )
    ...
    def create_payment_entry(self, invoice):
        ...
        frappe.msgprint(
            _("Payment Entry {0} created successfully.").format(payment.name)
        )